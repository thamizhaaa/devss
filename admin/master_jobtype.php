...
if(isset($_POST['submit'])) {
	...
	$e_query = mysql_query("select id from master_jobtype where jobtype='$jobtype'");
	$e_sq=mysql_num_rows($e_query);

	if ($jobtype == ""){
		 $err_jobtype = "Enter jobtype";
	}else if(!$e_sq=='') {
	...
	if(!$validation_form1) {
		...
		$indus_query = mysql_query("INSERT INTO  master_jobtype(usertype,jobtype)VALUES('".$session_usertype."','".$jobtype."')");
	?>
	<script>
	alert("Job type added Successfully");
	window.location = "master_jobtype.php";
	</script>
<?php
	}
}	
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>JobType | StaffingSpot</title>
...
<div class="panel-heading"><h3 class="panel-title">JobType Master</h3></div>
<div class="panel-body">
<form class="form_top_space" action="" method="post">

<label class="col-sm-3 control-label" style="margin-left:3%;">Job Type *</label>
<div class="col-sm-3"><input type="text"  class="form-control" name="jobtype" placeholder="Job Type *"  value="<?php echo $_POST['jobtype']; ?>"/><span class="help-block"><?php echo $err_jobtype; ?></span></div>
...
<th width="10%">SI NO</th><th >USER TYPE</th><th>JOB TYPE</th><th>OPTION</th>
<?php 

$sql=mysql_query("select * from master_jobtype");
$a = 0;
