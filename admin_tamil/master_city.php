...
<div class="panel-heading"><h3 class="panel-title">City Master</h3></div>
...
<select id="state" name="state" class="questions-category form-control state_allresults" data-placeholder="Select state">
...
<label class="col-sm-3 control-label" style="margin-left:3%;">City Name *</label>

<div class="col-sm-4">
<input type="text"  class="form-control" name="loc" placeholder="City Name *"  value="<?php echo $_POST['loc']; ?>"/>
...
<table class="table table-bordered table-responsive" align="center" width="100%" height="30px" border="1px" cellpadding="0" cellspacing="0">
    <th width="10%">SI NO</th><th>CITY</th><th>OPTION</th>
<!--<th>OPTION</th>-->
<?php 
$pagin_query =mysql_query("select count(*) as total  from  tbl_cities");
...
$sql=mysql_query("select * from tbl_cities WHERE fld_status=0 order by fld_id limit $k,$dis");
$a = $k+1;
...
<nav>
  <ul class="pager">
  <?php if($page_cur>1) { ?>
    <li class="previous"><a href="master_city.php?page=<?php echo ($page_cur-1);?>"><span aria-hidden="true">&larr;</span>Prev</a></li>
    <?php } else { ?>
    <li class="previous"><a>Prev</a></li>
    <?php } 
	if($page_cur<$total_page) {?>

    <li class="next"><a href="master_city.php?page=<?php echo ($page_cur+1); ?>">Next<span aria-hidden="true">&rarr;</span></a></li>
     <?php } else { ?>
